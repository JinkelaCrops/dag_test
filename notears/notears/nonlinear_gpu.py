...
from notears.trace_expm import trace_expm_gpu
import torch
...
print('is_torch_gpu_available=%s' % is_cuda)

device = torch.device('cuda:0' if is_cuda else 'cpu')
print(device)


class NotearsMLPGPU(nn.Module):
    def __init__(self, dims, bias=True):
        super(NotearsMLPGPU, self).__init__()
        assert len(dims) >= 2
        assert dims[-1] == 1
        d = dims[0]
        self.dims = dims
...
def notears_nonlinear(model: nn.Module,
    ...
    for _ in range(max_iter):
        rho, alpha, h = dual_ascent_step(model, X_cuda, lambda1, lambda2,
                                         rho, alpha, h, rho_max)
        if verbose > 0 and _ % verbose == verbose - 1:
            print('step %s, rho=%s, alpha=%s, h=%s' % (_ + 1, rho, alpha, h))
...
def main():
    ...
    n, d, s0, graph_type, sem_type = 2000, 20, 20, 'ER', 'mim'
    ...
    model = NotearsMLPGPU(dims=[d, 10, 1], bias=True)
